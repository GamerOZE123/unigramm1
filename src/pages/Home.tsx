...
export default function Home() {
  ...
  const fetchPosts = async (pageNum: number = 0, isInitial: boolean = false) => {
    ...
    try {
      ...
      if (postIds.length > 0) {
        const { data: startupData } = await supabase.from("student_startups").select("id, title").in("id", postIds);
      ...
      if (allPostsData) {
        rankedPosts = allPostsData.map((post) => ({
          ...
          startup:
            post.startup_id && startupsMap[post.startup_id] ? { title: startupsMap[post.startup_id].title } : undefined,
      ...
      if (pageNum === 0 && currentUserProfile) {
        ...
        if (currentUserProfile.university) {
          adsQuery = adsQuery.or(
            `target_universities.cs.{${currentUserProfile.university}},target_universities.is.null`,
          );
        }
