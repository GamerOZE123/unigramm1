...
// Simplified auction interface to avoid type conflicts
...
export default function AuctionPage() {
  ...
  const [selectedAuction, setSelectedAuction] = useState<SimpleAuction | null>(null);
  const [editingAuction, setEditingAuction] = useState<SimpleAuction | null>(null);
  const { user } = useAuth();
  ...
  const fetchAuctions = async () => {
    try {
      ...
      if (auctionsData && auctionsData.length > 0) {
        ...
        const auctionsWithData: SimpleAuction[] = auctionsData.map((auction: any) => ({
          id: auction.id,
          ...
          reserve_price: auction.reserve_price || auction.starting_price || 0,
  ...
  const canEditDelete = (auction: SimpleAuction) => {
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      {selectedAuction && (
        <AuctionDetailModal
          auction={selectedAuction as any}
