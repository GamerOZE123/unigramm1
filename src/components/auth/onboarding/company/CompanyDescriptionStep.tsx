...
export default function CompanyDescriptionStep({ onNext, onBack, onData, initialData }: CompanyDescriptionStepProps) {
  const [description, setDescription] = useState(initialData?.company_description || '');

  const handleNext = () => {
    if (!description) return;
    onData({ company_description: description });
    onNext();
  };
  ...
  return (
    <div className="space-y-6 bg-card p-8 rounded-lg shadow-lg">
      <div>
        ...
        <p className="text-muted-foreground">Describe what your company does</p>
      </div>
      ...
      <div>
        ...
        <Textarea
          ...
          placeholder="Tell students about your company, mission, and values..."
          rows={8}
