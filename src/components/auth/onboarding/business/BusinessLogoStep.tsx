...
interface BusinessLogoStepProps {
  onNext: () => void;
  onBack: () => void;
...
export default function BusinessLogoStep({ onNext, onBack, onData, initialData }: BusinessLogoStepProps) {
  const { user } = useAuth();
  ...
  return (
    <div className="space-y-6 bg-card p-8 rounded-lg shadow-lg">
      <div>
        <h2 className="text-2xl font-bold mb-2">Business Logo</h2>
        <p className="text-muted-foreground">Upload your business logo (optional)</p>
      </div>
